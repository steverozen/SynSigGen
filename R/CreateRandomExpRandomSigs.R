...
#
# Generate a suite of data with random signatures and exposures with
# specications requested by Ludmil, 2019 08 25
...
ParametersSALike <- function() {
  retval <- data.frame(
    ...
    mean.log10.exp.per.sig  = c(3.45, 3.0, 2.7,  2.8,  2.6),
    sd.log10.exp.per.sig    = c(0.4,  0.5, 0.5,  0.4,  0.4)
  )
...
GenerateAllRandomSA <-
  function(top.level.dir = "../SA.like.rand.exp.rand.sigs.2019.10.29",
           overwite = TRUE,
           verbose = TRUE,
           num.replicates = 10) {
    ...
    invisible(Resummarize(top.level.dir))
  }
...
ParametersSPLike <- function() {
  retval <- data.frame(
    ...
    mean.num.sigs.per.tumor = c(1.5, 3.9,  3.9,   3.9,  3.9),
    sd.num.sigs.per.tumor   = c(0.5, 1.1,  1.1,   1.1,  1.1),
    mean.log10.exp.per.sig  = c(3.8, 3.2,  3.3,   3.3,  3.3),
    sd.log10.exp.per.sig    = c(0.4, 0.35, 0.34,  0.34, 0.34)
  )
  return(retval)
}
...
GenerateAllRandomSP <-
  function(top.level.dir  = "../SP.like.rand.exp.rand.sigs.2019.10.29",
           overwrite      = TRUE,
           verbose        = TRUE,
           num.replicates = 10,
           unlink         = FALSE) {
...
Resummarize <- function(top.level.dir) {
  ...
  s4 <- do.call("rbind", s3)
  s.file <- file.path(top.level.dir, "summary2.csv")
  write.csv(s4, s.file)

  cat("This table has data rounded to 3 significant digits;",
      "see summary.csv for unrounded data",
      file = s.file, append = TRUE)

  cat("Synthetic data generated by ",
      "SynSigGenX::GenerateAllRandomSA or",
      "SynSigGenX::GenerateAllRandomSP on", date(), "\n\n",
      file = s.file, append = TRUE)

  cat("In real data\n",
      "mean total # mutations,17366\n",
      "sd,84673\n",
      "median,5259\n",
      "mad,5239\n\n",
      file = s.file, append = TRUE)

  utils::capture.output(sessionInfo(), file = s.file, append = TRUE)
  return(s4)
}
